...
using Plots,PlutoUI,CSV,GLM,Statistics,LinearAlgebra, Distributions, DataFrames, StatsBase,Lathe,MLBase,StatsPlots
...
begin
path = "https://github.com/pragya-gupta7/LinearReg-using-Julia/raw/main/insurance.csv"

# Download the CSV file
download(path, "insurance.csv")
	# Load the data into a DataFrame
data = CSV.read("insurance.csv", DataFrame)
end
...
# ╔═╡ ae5c43ce-8382-482a-8ef3-fa79b470e0b6
md"""
It's good practice to avoid spaces, special characters in the column names. In Julia, if column name has spaces, special characters, it is represented as a 'symbol'else it is prepended with a ":"
"""
...
boxplot(data.charges, title="BoxPlot: Charges",ylabel="Charges",legend=false)

# ╔═╡ b5f09f95-a54f-43df-83a3-ec487b97219b
md"""
There are few outliers in the data, represented by the points above the upper leaf. Let’s remove these data points so that linear regression works well
"""

# ╔═╡ abb90e3d-421a-41dc-92f8-89ab57de25fb
begin
first_percentile = percentile(data.charges, 25)
iqr_value = iqr(data.charges)
data! = data[data.charges .>  (first_percentile - 1.5*iqr_value),:];
end
...
density(data.charges , title = "Density Plot - Charges", ylabel = "Frequency", xlabel = "Charges", legend = false)

# ╔═╡ e82e3eab-f272-4b1a-b61e-cb549d14edb2
md"""
...
# ╔═╡ da15e286-2188-4ff3-a9fe-b39466e878b1
...
# ╔═╡ 6df9deab-be6b-4546-95c3-ca62209864fe
...
begin
	fm= @formula(charges~bmi+ children +age)
...
performance_testdf = DataFrame(y_actual = test[!,:charges], y_predicted = ypredicted_test)
...
performance_traindf = DataFrame(y_actual = train[!,:charges], y_predicted = ypredicted_train)
...
function cross_validation(train,k, fm = @formula(charges~ bmi))
    ...
    for i in 1:k
        ...
        performance_testdf = DataFrame(y_actual = temp_test[!,:charges], y_predicted = predict(linearRegressor, temp_test))
        performance_testdf.error = performance_testdf[!,:y_actual] - performance_testdf[!,:y_predicted]
...
cross_validation(train,15)

# ╔═╡ e9e3471e-c0ea-48cf-a3aa-e8af398b5e5d
cross_validation(test,15)
...
println("Correlation of Charges with BMI is ", cor(data.charges,data.bmi), "\n\n")

# Scatter plot
train_plot = scatter(data.bmi,data.charges, title = "Scatter Plot Charges vs BMI", ylabel = "Charges", xlabel = "BMI",legend = false)
end
  ╠═╡ =#
...
julia_version = "1.9.0"
manifest_format = "2.0"
project_hash = "b765da764604075a5440968f5dda80f2199a61ed"
